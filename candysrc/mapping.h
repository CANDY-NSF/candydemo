...
Struct mappingZeroIndexing(vector<int_int> initialGraphVector) {
  ...
  //    const auto p = std::minmax_element(initialGraphVector.begin(),
  //    initialGraphVector.end()); auto min = p.first->first; auto max =
  //    p.second->first; std::cout << "min=" << min << ", max=" << max <<
  //    std::endl;
  ...
  //    for(int i=0;i<initialGraphVector.size();i++)
  //    {
  //        cout<<initialGraphVector.at(i).first<<"
  //        "<<initialGraphVector.at(i).second<<"\n";
  //    }

  outputStruct.outputGraphVector.push_back(std::pair<int, int>(
      outputStruct.graphMap.at(max) + 1, initialGraphVector.size()));

  for (int i = 0; i < initialGraphVector.size(); i++) {
    outputStruct.outputGraphVector.push_back(std::pair<int, int>(
        outputStruct.graphMap.at(initialGraphVector.at(i).first),
        outputStruct.graphMap.at(initialGraphVector.at(i).second)));
  }
...
int mapKey(Struct outputStruct, int someValue) {
  for (const auto &[key, value] : outputStruct.graphMap)
    if (value == someValue)
      return key;
}
