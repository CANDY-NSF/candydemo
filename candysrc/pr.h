...
void compute_dValue(PR_Network *X, int *p) {
  ...
  for (int i = 0; i < X->size(); i++) {
    ...
    if (X->at(i).In_Neigh.size() == 0) {
      X->at(i).dValue = X->at(i).page_rank * X->size();
    } else {
      ...
      for (int j = 0; j < X->at(i).In_Neigh.size(); j++) {
        ...
        sum = sum +
              ((float)X->at(neigh).page_rank / (float)X->at(neigh).Out_size);
      }
...
/*
     * PR(v)= (d(given in preprocessing file )/n(number of nodes in the network)
   + (1-d)*[PR(v1)/O(V1)+PR(v2)/)(V2)])

     *   PR(v1)/O(v1)+PR(v2)/ O(v2) == Value which is stored in the meta
...
void updateinitialIdentified(PR_Network *X, int *p, int *maxL,
...
#pragma omp parallel for schedule(dynamic) num_threads(*p)
  ...
  for (int i = 0; i < initialList->size(); i++) {
    ...
    for (int j = 0; j < neighborRevisedList->at(initialList->at(i))
                            .subtractionNeighborList.size();
         j++) {

      new_pr = new_pr - ((float)X
                             ->at(neighborRevisedList->at(initialList->at(i))
                                      .subtractionNeighborList.at(j))
                             .page_rank /
                         (float)X
                             ->at(neighborRevisedList->at(initialList->at(i))
                                      .subtractionNeighborList.at(j))
    ...
    for (int j = 0; j < neighborRevisedList->at(initialList->at(i))
                            .additionNeighborList.size();
         j++) {

      new_pr = new_pr + ((float)X
                             ->at(neighborRevisedList->at(initialList->at(i))
                                      .additionNeighborList.at(j))
                             .page_rank /
                         (float)X
                             ->at(neighborRevisedList->at(initialList->at(i))
                                      .additionNeighborList.at(j))
      ...
      if (X->at(i).updateFlag == true) {
        ...
        for (int j = 0; j < X->at(i).In_Neigh.size(); j++) {
          if (X->at(i).In_Neigh[j].second != -1) {
            ...
            sum = sum + ((float)X->at(neigh).page_rank /
                         (float)X->at(neigh).Out_size);
            // cout <<i<<":::"<<neigh<<":::"<<X->at(neigh).Out_size<<"\n";
          }
        } // end of for
...
void removeDanglingVertices(PR_Network *X, int *p) {
  ...
  // long * vertexMarkedForUpdateAfterEachIteration = (long *)malloc
  // (X->size()*sizeof(long));
