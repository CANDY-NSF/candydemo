...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    cout << "INPUT ERROR:: 2 inputs required. First: filename \n Second: file "
...
#pragma omp parallel for num_threads(p)
  ...
  // stop when (i) all connectors have same (max id) OR (ii) all vertices are
  // reached--which ever ends earlier.
...
#pragma omp parallel for num_threads(p)
    for (int i = 0; i < X.size(); i++) {
      ...
      // printf("%d --%d--%d--%d--%d \n",i, f1,
      // Cmax[f1],V_Info[i].fw,V_Info[i].bw );

      // check if i is a connector and it reached the max color
      // If not set change2=true for further iterations
      if (V_Info[i].is_conn) {
        if (V_Info[i].fw < max_col) {
          change2 = true;
          Cdone[f1] = false;
        }
        if (V_Info[i].bw < max_col) {
          change2 = true;
          Cdone[f1] = false;
        }
      }

      // if(Cdone[f1]==true) {continue;} //no need to process if work on
      // component completed

      if (V_Info[i].is_done) {
        continue;
      } // no need to process for known components

      // propagate only if value received
      if (V_Info[i].fw > -1) {
        int edg = 0;
        for (int j = 0; j < X[i].In_ListW.size(); j++) {
          if (X[i].In_ListW[j].second == 0) {
            continue;
          } // do not propagate through deleted edge
      ...
      // If vertex obtained highest value from both fw and bw then component
      // found
      if ((V_Info[i].fw == Cmax[f1]) && (V_Info[i].bw == Cmax[f1])) {
        V_Info[i].is_done = 1;
      }
  ...
  // Once the components are found--find the components where fw and bw are
  // different
  bool changeX = true;
  while (changeX) {
    changeX = false;
...
#pragma omp parallel for num_threads(p)
  ...
  /*   for(int i=0;i<X.size();i++)
     ...
     V_Info[i].fw1, V_Info[i].bw1, V_Info[i].is_done, V_Info[i].lcl_ID,
     V_Info[i].glb_ID);
...
#pragma omp parallel for num_threads(p)
  ...
  /* for(int i=0;i<X.size();i++)
   {
       printf("%d  %d %d|| :%d %d %d\n", i, V_Info[i].fw, V_Info[i].bw,
   V_Info[i].is_done, V_Info[i].lcl_ID, V_Info[i].glb_ID);
   }*/
