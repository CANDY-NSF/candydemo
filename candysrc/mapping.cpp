...
//mapping("file_0.txt","map.txt","output.txt");
    ...
    {
        cout<<outputValues.outputGraphVector.at(i).first<<" "<<outputValues.outputGraphVector.at(i).second<<"\n";
    }


    for(int i=0;i<outputValues.outputGraphVector.size();i++)
    {
        outputafterReverseSCCmap.push_back(std::pair<int, int>(mapKey(outputValues,outputValues.outputGraphVector.at(i).first),outputValues.outputGraphVector.at(i).second));
    }
